    ...
    private function saveToDB($sql_result, $cart, $total, $data, $currencyid, $customer) {
        ...
        if (empty($sql_result) && Configuration::get('BILLPLZ_IPNMODE')) {
            ...
            $this->module->validateOrder(
                    $cart->id, Configuration::get('PS_OS_PAYMENT'), $total, $this->module->displayName, '(IPN Callback) Billplz Bills URL: <a href="' . $data['url'] . '" target="_blank">' . $data['url'] . '</a>', null, (int) $currencyid, false, $customer->secure_key
            );
